...
# ================= 配置区域 =================
# 模型名称 (ProGuard-7B)
MODEL_ID = "/HDD0/zhangyuheng/safety_analysis/ProGuard-7B" 

# 数据集配置
DATASETS = {
    "mm_safetybench": {
        "root": "/HDD0/zhangyuheng/safety_analysis/mm_safetybench",
        "output": "mm_safetybench_proguard_results",
...
def run_inference(dataset_name):
    ...
    # 修复配置文件问题：添加缺失的 vision_start_token_id 和 vision_end_token_id
    if hasattr(model.config, 'text_config'):
        if hasattr(model.config.text_config, 'vision_start_token_id') and not hasattr(model.config, 'vision_start_token_id'):
            ...
            print(f"已设置 vision_start_token_id: {model.config.vision_start_token_id}")
        ...
        if hasattr(model.config.text_config, 'vision_end_token_id') and not hasattr(model.config, 'vision_end_token_id'):
            ...
            print(f"已设置 vision_end_token_id: {model.config.vision_end_token_id}")
        ...
        if hasattr(model.config.text_config, 'image_token_id') and not hasattr(model.config, 'image_token_id'):
            ...
            print(f"已设置 image_token_id: {model.config.image_token_id}")
        ...
        if hasattr(model.config.text_config, 'video_token_id') and not hasattr(model.config, 'video_token_id'):
            ...
            print(f"已设置 video_token_id: {model.config.video_token_id}")
            ...
            for idx in tqdm(range(start_idx, len(eval_data)), total=len(eval_data)-start_idx):
                try:
                    ...
                    if 'image' in item:
                        ...
                        image_filename = f"img_proguard_{dataset_name}_{category}_{subdir}_{idx}.jpg"
...
if __name__ == "__main__":
    # 解析命令行参数
    parser = argparse.ArgumentParser(description="运行 ProGuard 模型推理")
    parser.add_argument(
        "--dataset", 
        type=str, 
